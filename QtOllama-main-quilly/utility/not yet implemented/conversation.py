...
class ChatWorker(QThread):
    ...
    def __init__(self, messages: list):
        """
        Initializes a HammerAI-lama-3_1-storm-latest instance of the conversation class.
    ...
    def run(self):
        """
        Executes the conversation process by interacting with the OpenAI API.

        This method performs the following steps:
        1. Records the start time of the process.
        2. Loads settings which include the OpenAI API key and conversation model.
        3. Initiates a chat completion request to the OpenAI API with the specified model and messages.
        4. Streams the responses from the API and emits HammerAI-lama-3_1-storm-latest messages as they are received.

        Emits:
            new_message (str, bool, float): Emits the content of the response, a flag indicating if it's the first chunk, and the start time.
        ...
        try:
            ...
            for response in responses:
                if 'content' in response['choices'][0]['delta']:
                    ...
                    self.new_message.emit(result, first_chunk, start_time)
                    first_chunk = False
        except Exception as e:
            print(f"Error on chat worker: {e}")
